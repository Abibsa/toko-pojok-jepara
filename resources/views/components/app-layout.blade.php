...
<head>
    ...
    <meta name="description" content="{{ $metaDescription ?? 'Toko Pojok Jepara - Toko grosir terpercaya dengan sistem stok real-time dan harga terbaik untuk kebutuhan bisnis Anda.' }}">
...
<body class="font-sans antialiased bg-gray-950 text-gray-100 min-h-screen">
                ...
                <div class="flex justify-between h-20">
                    ...
                    <div class="flex items-center space-x-3">
                        ...
                        @auth
                            ...
                            <!-- Notifications -->
                            <a href="{{ route('pesanan.index') }}" class="relative p-2.5 text-gray-400 hover:text-white hover:bg-white/5 rounded-full transition-all duration-300" aria-label="Notifikasi">
                                <svg class="w-6 h-6" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                            ...
                            <!-- User Menu -->
                            <div class="relative">
                                <button id="user-menu-btn" class="flex items-center space-x-2 px-3 py-2 rounded-full bg-white/5 hover:bg-white/10 transition-all duration-300 border border-white/10" aria-label="Menu Pengguna">
            ...
            <div id="mobile-menu" class="hidden md:hidden border-t border-white/10">
                <div class="px-4 py-4 space-y-2">
                    ...
                    <form action="{{ route('produk.index') }}" method="GET" class="relative mb-4">
                        <input type="text" name="search" value="{{ request('search') }}" 
                               placeholder="Cari produk..." 
                               class="w-full px-4 py-3 pl-10 bg-gray-800/50 border border-white/10 rounded-xl text-sm text-gray-300 placeholder-gray-500 focus:outline-none focus:ring-2 focus:ring-red-500/50 focus:border-transparent">
                        <svg class="absolute left-3 top-3.5 w-5 h-5 text-gray-500" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M21 21l-6-6m2-5a7 7 0 11-14 0 7 7 0 0114 0z"></path>
                        </svg>
                    </form>

                    <a href="{{ route('home') }}" class="block px-4 py-3 text-sm font-medium rounded-xl {{ request()->routeIs('home') ? 'text-white bg-white/10' : 'text-gray-300 hover:text-white hover:bg-white/5' }} transition-all duration-200">
                        Beranda
                    </a>
                    <a href="{{ route('produk.index') }}" class="block px-4 py-3 text-sm font-medium rounded-xl {{ request()->routeIs('produk.*') ? 'text-white bg-white/10' : 'text-gray-300 hover:text-white hover:bg-white/5' }} transition-all duration-200">
                        Produk
                    </a>
                    @auth
                        <a href="{{ route('pesanan.index') }}" class="block px-4 py-3 text-sm font-medium rounded-xl {{ request()->routeIs('pesanan.*') ? 'text-white bg-white/10' : 'text-gray-300 hover:text-white hover:bg-white/5' }} transition-all duration-200">
                            Pesanan
                        </a>
                    @endauth
                </div>
        ...
        <main class="relative">
            @if(session('success'))
                <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 pt-6">
                    <div class="flash-message backdrop-blur-xl bg-green-500/10 border border-green-500/30 text-green-400 px-6 py-4 rounded-2xl shadow-lg shadow-green-500/20 animate-slide-up">
                        <div class="flex items-center justify-between">
                            ...
                            <button onclick="this.parentElement.parentElement.remove()" class="text-green-400 hover:text-green-300" aria-label="Tutup pesan">
            ...
            @if(session('error'))
                <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 pt-6">
                    <div class="flash-message backdrop-blur-xl bg-red-500/10 border border-red-500/30 text-red-400 px-6 py-4 rounded-2xl shadow-lg shadow-red-500/20 animate-slide-up">
                        <div class="flex items-center justify-between">
                            <div class="flex items-center">
                                <svg class="w-5 h-5 mr-3" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                            ...
                            <button onclick="this.parentElement.parentElement.remove()" class="text-red-400 hover:text-red-300" aria-label="Tutup pesan">
                                <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M6 18L18 6M6 6l12 12"></path></svg>
                            </button>
                        </div>
                    </div>
    ...
    <script>
        ...
        // Show notification (single unified version)
        function showNotification(message, type = 'success') {
            // Remove existing notifications to prevent stacking
            document.querySelectorAll('.js-notification').forEach(n => n.remove());
            
            const notification = document.createElement('div');
            notification.className = `js-notification fixed top-24 right-4 p-4 rounded-2xl shadow-2xl z-50 backdrop-blur-xl border animate-slide-up transition-all duration-300 ${
                type === 'success' 
        ...
        document.addEventListener('DOMContentLoaded', function() {
            ...
            // Auto-hide flash messages (only target .flash-message, not JS notifications)
            setTimeout(() => {
